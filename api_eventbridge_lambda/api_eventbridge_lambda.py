...
class ApiEventBridgeLambdaStack(Stack):
    ...
    def __init__(self, scope: Construct, id: str, **kwargs) -> None:
        ...
        # sqs_policy = iam.PolicyStatement(effect=iam.Effect.ALLOW, resources=['*'], actions=['sqs:SendMessage'])
        # event_producer_lambda.add_to_role_policy(sqs_policy)
        event_consumer2_rule = events.Rule(self, 'eventConsumer2LambdaRule',
                                           description='Approved Transactions',
                                           event_pattern=events.EventPattern(source=['com.mycompany.myapp']
                                                                             ))
        event_consumer2_rule.add_target(targets.SqsQueue(queue))
        sqs_event_source = lambda_event_source.SqsEventSource(queue)
        #
        # Approved Consumer2
        #
        sqs_lambda = _lambda.Function(self, "eventConsumer2Lambda",
                                                  runtime=_lambda.Runtime.PYTHON_3_8,
                                                  handler="event_consumer_lambda.lambda_handler",
                                                  code=_lambda.Code.from_asset("lambda")
                                                  )
        ...
        # ingest_bucket = s3.Bucket(self, 'test-ngest-bucket-bp')
